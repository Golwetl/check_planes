...
bool angle_plane(pcl::PointXYZ pt, pcl::ModelCoefficients::Ptr coefficients){
    if ( abs(coefficients->values[0]) > (abs(coefficients->values[2]) + abs(coefficients->values[1]))*7){
        return true;
    } else{
    return false;
}
...
double x_distnace(pcl::PointXYZ pt, pcl::ModelCoefficients::Ptr coefficients){
    double f1 =  pt.z;
    return f1;
}

double y_distnace(pcl::PointXYZ pt, pcl::ModelCoefficients::Ptr coefficients){
    double f1 = pt.y;
    return f1;
}
...
public:
    ...
    void getColor(double c,bool ang_plane,double error_x,double error_y,pcl::PointXYZ pt ,uint8_t& R, uint8_t& G, uint8_t& B){
    ...
    void getColor(double c,bool ang_plane,double error_x,double error_y, pcl::PointXYZ pt ,double &rd, double &gd, double &bd){
        uint8_t r;
        uint8_t g;
        uint8_t b;
        getColor(c,ang_plane,error_x,error_y,pt,r,g,b);
        rd = (double)r/255;
        gd = (double)g/255;
        bd = (double)b/255;
    }
    uint32_t getColor(double c,bool ang_plane,double error_x,double error_y ,pcl::PointXYZ pt){
        uint8_t r;
        uint8_t g;
        uint8_t b;
        getColor(c,ang_plane,error_x,error_y,pt,r,g,b);
...
public:
    ...
    void initialize(){
        ...
        // Subscriber
        _subs = _nh.subscribe("/sim/velodyne_points",1,&planeFitter::pointCloudCb,this);

        // Get parameters
        ros::param::param<double>("~max_distance",_max_distance,0.001);
        ros::param::param<double>("~min_percentage",_min_percentage,1);
    ...
    void pointCloudCb(const sensor_msgs::PointCloud2::ConstPtr &msg){
        ...
        while (cloud->height*cloud->width>original_size*_min_percentage/100){
            ...
            for (int i=0;i<inliers->indices.size();i++){
                ...
                if (min_x > pt.z)   min_x = pt.z;
                if (max_x < pt.z)   max_x = pt.z;
            ...
            for (int i=0;i<inliers->indices.size();i++){
                ...
                error_x = 1;
                error_y = 1;
                // Check the border our planes
                if (pt.y < max_y - 1   && pt.y > min_y + 1 && pt.z < max_x - 1 && pt.z > min_x + 1 )
                    error_x = -1;
                double ang_plane = angle_plane(pt,coefficients);
                uint32_t rgb;
                if (_color_pc_with_error)
                    rgb = cm.getColor(err[i],ang_plane,error_x,error_y,pt);
                else
